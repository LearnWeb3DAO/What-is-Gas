...
The way these wallets are able to _provide reasonable estimates for gas prices based on current network conditions_ is by looking at the average gas fees other users are paying in the network right now.
You don't want to send a transaction with less gas fees than that, as it may significantly reduce your chances of the transaction being mined (miner has higher paying txn options), and you (usually) don't want to pay more gas fees than that as you can get your transaction mined for cheaper, unless you want it to be fast!
These gas estimations change in real time according to the network conditions. [This is one of the websites where you can see them change :)](https://etherscan.io/gastracker)
...
In simpler words, this means that Ethereum, as a currency, can be/is now (deflationary)[https://cointelegraph.com/news/ethereum-sees-first-consecutive-week-of-deflationary-issuance] (tokens that decrease in total supply each time a token transfer happens).
...
[These tradeoffs and/or scaling techniques are disucussed in what is called The Blockchain Trillema](https://www.ledger.com/academy/what-is-the-blockchain-trilemma)
